...
print("\nWelcome to AStar, by YOUR NAME HERE!")
...
#         and if it is already on CLOSED, delete the new instance.
    ...
    for op in Problem.OPERATORS:
      if op.precond(S):
        ...
        f_new_state = new_state_cost + h(new_state)
        L.append((new_state, f_new_state))

    for item in L[:]:
      if item[0] in CLOSED:
        if item[1] > CLOSED[item[0]]:
          L.remove(item)
        else:
          del CLOSED[item[0]]
      elif OPEN.__contains__(item[0]):
        if item[1] > OPEN.__getitem__(item[0]):
          L.remove(item)
        else:
          OPEN.__delitem__(item[0])
    
# Step 5
    for sprime, f_sprime in L:
        OPEN.insert(sprime, f_sprime)
